...
export default function CreditAnalytics() {
  ...
  const loadCreditData = async () => {
    try {
      ...
      const credits = history.filter(t => t.type === 'Credit');
      setCreditTxns(credits);

      // Compute chart data: Group credits by last 7 calendar days
      const days = [];
      const now = new Date();
      for (let i = 6; i >= 0; i--) {
  ...
  const filteredCredits = creditTxns.filter(txn => {
    ...
    return (
      ...
      fromAcc.toLowerCase().includes(q) ||
      txn.amount.toString().includes(q)
    );
  });

  // Calculate totals
  const totalCreditSum = creditTxns.reduce((sum, t) => sum + t.amount, 0);
      ...
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '2rem', marginBottom: '3rem' }}>
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#6EE7B7', marginBottom: '0.75rem' }}>
              <TrendingUp size={22} />
              <span style={{ fontSize: '0.85rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>CREDIT ANALYTICS</span>
            </div>
            <h2 style={{ fontSize: '1.85rem', fontWeight: 800 }}>{totalCreditSum.toLocaleString()} INR</h2>
            <p style={{ color: '#94A3B8', fontSize: '0.85rem', marginTop: '0.5rem' }}>Total accumulated credit inflows over active ledger timeline.</p>
          </div>
          ...
          <div style={{ 
            ...
            <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--white)', marginBottom: '1.5rem' }}>Daily Credit Volume (Last 7 Days)</h3>
        ...
        <div className="statement-section">
          ...
          <div className="section-header" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
            <h3 className="section-title">Chronological Credit Records</h3>
            ...
            <div style={{ position: 'relative', width: '100%', maxWidth: '300px' }}>
              ...
              <input
                ...
                placeholder="Search sender, ID, or amount..."
                value={searchQuery}
          ...
          <div className="table-container">
            {filteredCredits.length > 0 ? (
                  ...
                  {filteredCredits.map((txn) => {
                    ...
                    return (
                      <tr key={txn._id}>
                        ...
                        <td style={{ fontFamily: 'monospace', fontSize: '0.85rem' }}>{fromAcc}</td>
                        <td>Fund Transfer Received</td>
                        <td>
                          <span style={{ 
                            fontSize: '0.65rem', 
                            fontWeight: 700, 
                        ...
                        <td className="amount-credit" style={{ fontWeight: 700 }}>
              ...
              <div className="no-account-card" style={{ padding: '4rem 2rem' }}>
                ...
                <h4 style={{ fontWeight: 700, marginTop: '0.5rem' }}>No Matching Credits</h4>
